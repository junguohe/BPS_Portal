...
Ext.define('BPSPortal.view.returngoods.InventoryAdjustment.InventoryAdjustmentViewController', {
    extend: 'Ext.app.ViewController',
    alias: 'controller.InventoryAdjustment',
    ...
    onAfterRenderW:function(){
	    ...
	    onExportClicks:function(button){
	    	var grid = Ext.ComponentQuery.query("viewport InventoryAdjustment grid")[0];
    ...
    onSearchClick:function(btn){
        ...
        /*传name到后台查询*/
        var extraParams = {period:period,dealername:dealername,taskconfirm:taskconfirm,taskcontent:'inventoryadjustment'};
    ...
    onDeleteClick:function(button){
        ...
        Ext.Msg.confirm("删除", content, function (btn) {
            if (btn == "yes") {
            	var param = {tid:tid,type:'inventoryadjustment'}; 
    ...
    onDetailClick:function(btn){
    	...
    	var vmData = {};
    	   vmData.data = records;
	        ...
	        this.fireViewEvent('viewObject', this.getView(), 'inventoryadjustmentdetail', vmData);
    ...
    onAfterRenders:function(){
    	...
    	var tid=records.data.id;
    	  var grid = me.lookupReference('uploadinventoryadjustmentGrid'),
              store = me.getViewModel().getStore("uploadinventoryadjustment");
    	    var paras = { tid: tid};
    ...
    onsubmitClick:function(button){
        var me = this;
    	var adjustment=[];
    	var grid=this.lookupReference('uploadinventoryadjustmentGrid');
     	var store = grid.getStore();
	    for (var i = 0; i < store.getCount(); i++) {
	    	var records = Ext.create('BPSPortal.model.UploadInventoryAdjustment');
	    	...
	    	records.data.refdealer =store.getAt(i).data.refdealer;
//	    	if(arr[2].equals("入")||arr[2].equals("转入")){
//				dto.setAdjtype("1");
//			}else{
//				dto.setAdjtype("2");
//			}
	    	records.data.adjtype =store.getAt(i).data.adjtype;
	    	console.log(records.data.adjtype);
	    	...
	    	adjustment.push(records.data);
	        }
	       	var param = {adjustment:adjustment}; 
	       	Ext.MessageBox.wait("正在执行操作......", "提示");
	       	uploadinventoryadjustmentcontroller.SaveOrCommit(param,function(result){ 
       	    if(result.success){
       	    	Ext.MessageBox.hide();
       	    	Ext.toast({
	                    title: 'Tips', html: result.msg, align: 't',
